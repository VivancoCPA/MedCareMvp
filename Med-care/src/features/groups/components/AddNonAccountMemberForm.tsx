...
import { useAddNonAccountMember } from '../hooks/useAddNonAccountMember'
...
export function AddNonAccountMemberForm({ groupId, onDone }: AddNonAccountMemberFormProps) {
  const { t } = useTranslation()
  const { addNonAccountMember, isPending } = useAddNonAccountMember(groupId)
    ...
    <form className="space-y-4" onSubmit={form.handleSubmit(handleSubmit)}>
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col gap-2">
          <Label htmlFor="nonaccount-avatar">{t('groups.addMember.nonAccount.avatarLabel')}</Label>
          <input
            ref={fileInputRef}
            id="nonaccount-avatar"
            type="file"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          <Label htmlFor="nonaccount-firstName">{t('groups.addMember.nonAccount.fields.firstName')}</Label>
          <Input id="nonaccount-firstName" {...form.register('firstName')} />
          {form.formState.errors.firstName && (
            <p className="text-xs text-destructive">{form.formState.errors.firstName.message}</p>
          )}
        </div>
        <div className="space-y-1">
          <Label htmlFor="nonaccount-lastName">{t('groups.addMember.nonAccount.fields.lastName')}</Label>
          <Input id="nonaccount-lastName" {...form.register('lastName')} />
        </div>
      </div>

      <div className="space-y-1">
        <Label htmlFor="nonaccount-memberType">{t('groups.addMember.nonAccount.fields.memberType')}</Label>
        <Select value={memberType} onValueChange={handleMemberTypeChange}>
          <SelectTrigger id="nonaccount-memberType">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          <Label htmlFor="nonaccount-birthDate">{t('groups.addMember.nonAccount.fields.birthDate')}</Label>
          <Input id="nonaccount-birthDate" type="date" {...form.register('birthDate')} />
        </div>
        <div className="space-y-1">
          <Label htmlFor="nonaccount-gender">{t('groups.addMember.nonAccount.fields.gender')}</Label>
          <Select
            value={form.watch('gender')}
            onValueChange={(value) => form.setValue('gender', value as Gender, { shouldValidate: true })}
          >
            <SelectTrigger id="nonaccount-gender">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {isHuman ? (
      ...
      {memberType === 'pet' && (
        <div className="space-y-1">
          <Label htmlFor="nonaccount-breed">{t('groups.addMember.nonAccount.fields.breed')}</Label>
          <Input id="nonaccount-breed" {...form.register('breed')} />
        </div>
      )}

      <div className="space-y-1">
        <Label htmlFor="nonaccount-bloodType">{t('groups.addMember.nonAccount.fields.bloodType')}</Label>
        <Select
        ...
        >
          <SelectTrigger id="nonaccount-bloodType">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="flex justify-end">
        <Button type="submit" disabled={isPending}>
          ...
          {t('groups.addMember.nonAccount.submit')}
        </Button>
      </div>
    </form>
  )
