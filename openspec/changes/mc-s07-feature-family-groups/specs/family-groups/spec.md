...
#### Scenario: Doctor and center/pharmacy steps are layout-only in this change
- **WHEN** the Admin views the doctors or centers/pharmacies onboarding step
- **THEN** only the step's layout and skip action render — no real repository-selection logic is present, since that belongs to a separate change
